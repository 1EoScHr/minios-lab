实验记录

# Lab1

## 环境的配置

在开始**Lab1**之前，需要根据**实验环境说明书**配置出一套符合要求的环境。  
本来以为这就和配置网课的xv6一样**简单**，无非是另外安装qemu的x86库就行，**但是**安装时却出了问题，RISC-V的工具链要**被强制删除**，原因是xv6依赖的**risc-v编译器**与本实验需要的**32位程序编译器**冲突，可以认为是它们各自用到的gcc12版本不一样（是两个变体）。  

搜索了一下，最好的解决办法就是将两个环境**隔离**起来，并且使用**docker**会更方便。之前一直听说docker的好处，但没有遇到过必须用其解决的情况，所以正好趁此机会大体的了解了一下docker，并用它构建一个满足本实验编译的环境来练手，具体的描述[记在了这里]()。  

后面尝试用boot启动虚拟机时，发现需要用到**GUI**，但是容器里没有图形化界面，再装一个有点不值；再看了一下xv6的实现并不需要用到图形界面，所以就重新弄了一下，把xv6的RISC-V编译链搬到docker里（具体操作放到那边的文档），本课的实验放在**宿主机**上就好。  

## boot.asm

TODO完成起来没有很难，主要是**看文档**比较费事，先是在**int 10**里面找了半天能够显示字符串的分类，找到后又继续研究里面各个寄存器是用来干什么的。  
好在确定是**int 10**中断的**AH=13h**类型后，就能确定**实验指导**给的表的意思了，因此完成起来不难，对照着填寄存器的值就行。最后的效果：  
![](pictures/hios.png)  

### 求甚解

尽管基本任务完成了，但还是有些地方其实一无所知。需要弄懂。  

#### org

刚开始的`org 0x7c00`命令，后面的地址是约定的boot程序起始段（BIOS会把引导程序读到此位置，所以实际执行时是以此地址为起始的），但前面的命令在ppt给的常见命令中没见到。  

**org**即**origin**，作用是改变**汇编器**内部的地址计算方式，并不会生成机器码。  
也就是汇编器默认代码的起始内存是0，但实际上是0x7c00，如果不提示汇编器这一点，其在执行**跳转**等命令时就会算错偏移量。  

#### 标签

代码一共有两个标签，分别是**DispStr**与**BootMessage**，那为什么一个就能包括代码段，一个就只是定义了一个字符串？  

标签本身只是一个**地址符号**，记录了其所在的这一行的位置，并没有包括或不包括后面的代码。之所以两个标签的效果不同，是因为**DispStr**是被**call**使用的，会跳转到其位置运行，直到读到**ret**返回；但是**BootMessage**只是被当作一个数据地址。  

#### times   510-($-$$) db 0

这里没有看懂，只知道注释里写的是填充区域的。  

（此时整个扇区可以认为是一个“段”）  
+ `times <count> db <value>`指令的作用是重复执行`db <value>`count次
+ `$`表示当前的**汇编位置**，其起始点会受**org**命令的影响
+ `$$`表示当前**段的起始位置**，默认是0，但会受**org**的影响改变

所以`$-$$`得到的就是**段内偏移量**，在本例中是得到从第一条实际执行的指令开始到当前位置的字节数。  

在此之上，写成`510-....`就能刚好将扇区剩余空间补0补到510字节，配合后面的两字节结束码，**凑齐512B**，符合**BIOS的引导规范**。  
（那两字节的**0xaa55**也属于是**早期约定规范**，叫做**引导扇区签名**，BIOS在512B的最后读到这两个字节，才会认定是一个符合要求的boot程序）  

#### 最后两行永远跑不到

确实如此，从**程序运行**角度看，的确到不了；  
但是**程序本身也是数据**，虽然跑不到，但是其作为数据被BIOS检查时这里就会用到。  

#### 为何页数改动看不见、为何文字显示在第一行的一半

以上两个问题是对于中断要求的相关寄存器配置。  

页数对应的是bh寄存器，为0时能看到，其他时看不到，是因为启动时候只有这么一页；  
文字显示是默认跟着光标走，这说明光标位置初始不在此。可以通过别的中断来设置（好像也是后面的任务）  

### 总结流程

电脑启动，固件中的bios程序开始执行，首先对硬件进行自检，然后检测启动盘（在qemu中的-driven参数），并按下面顺序：把启动盘一共512B的数据放到内存的0x7c00处，再检查最后的2字节是否符合要求，符合就从0x7c000处按照此boot程序加载系统，否则换下一个启动盘尝试。  

## 滚动窗口

清屏这一需求，自然是提到的**06h**中断，其能够把选中的窗口整体上移（或直接清空）。  

描述是这么简单，但是**实现起来有点晕**：这一整屏幕，怎么算行、怎么算列？  

询问了一下GPT，答案是**绝大多数 BIOS 默认启动进入的就是 80 列 × 25 行文本模式**，其他模式还可以通过别的中断设置。  

这样就能继续了。  

***

# Lab2

Lab2一上来就似乎有点上强度了，看的晕晕乎乎。  

## 先运行起来

既然看不懂，那就先把默认的运行起来再说。  
跟着指导书后面的流程，进行**disk.img**的构建：  
+ 系统没有**mkfs**命令，查找可知其在某些对应的包里，通过`sudo apt install util-linux dosfstools`安装，可是显示已经安装；那么可能就是在root用户下了，查看了一下，果然。  
+ 指导书中给定了“不安全”的挂载方式，是到**/mnt**，并在提示中说明了Makefile中给定了更安全的方式，那就是创建一个临时目录并挂载到其上。  

最后把按照教程制作好的**disk.img**挂载并运行，却发现连“Hello,World!"都没有输出，而是乱码：  
![](pictures/1.png)  

可能是我的操作细节不对？那就用给定的**Makefile**进行make，来看看能出来不。  

但是却报了错，在82行、错误127，这意味着**命令未找到**，定位到82行语句为`	@mkfs.vfat -F 32 -s 8 $@ > /dev/null 2>&1`，果然是因为我这里只有root才有mkfs的相关命令。所以在开头加上一个**sudo**，这样就好了。  

用qemu挂载这个新镜像，竟然出现了正确的效果：  
![](pictures/2.png)  

看来的确是自己手操的步骤里哪里不规范。  
不过教程里确实都一直提到只是一个概念，具体的、安全的一些方式都能在Makefile中找到，那就先这样，后面做起来再结合Makefile看看正确的做法都是什么。  

## 任务1

### 单引号的表示

步骤1中的第1条，是一个照猫画虎的更改，在**loader.asm**里观察，发现**WriteChar**就类似一个函数，接收行号、列号与要显示的字符（见下一条）。  

在编排`This is lzt's loader.`时，对“**'**”这一符号犯难，试图将其用转义符，构建时会报错。经过一番查找，得知用**ASCLL码**(39or0x27)或者**双引号包围**("'")即可，nasm并不支持转义符的这种风格。  

#### marco 宏

在**loader.asm**中，创建了一个名为**WriteChar**的宏，可以理解为是一个宏函数。  

然后了解一下具体的语法，就是通过`%marco`与`%endmarco`包裹，然后是**宏名+参数数量**，以及具体实现，就是一个编译期间自动进行的文本替换。  

### 手动写入纠错

这一步骤是必须要手动写入，那么就不用make了，要搞明白为什么自己手动写入的不行。  

先尝试用**diff**命令分析一下不同两种方式得到的文件不同之处，但是其对于**.img**这种二进制文件，只能输出是否相同，却不能输出哪里不同。查找资料，得知可以用**xxd**命令来将二进制文件转为十六进制文件，就可以使用diff了。  

于是分别把手动写入与make自动构造出的的**disk.img**转成**mydisk.img**与**makedisk.img**，再用diff分析，但输出的结果没有办法看，没有规律、数量巨大。  

于是只好在**vsCode**中打开，手动分析一下两文件的差异：  
![](pictures/3.png)  

在开头处，左侧为自己手动编译的，右侧为make编译出来的，很容易发现两边在翻译成“**LOADER BINBooting……**”后都是一致的，但是前面这里不同，左边像是直接覆盖了某些字段，导致发生了一定的错位，一直到0x200。根据翻译出的字段**.X.mkfs.fat**以及手册中提到的“*实际上由于 fat32 在第一个扇区保留了 fs 的元信息，故 boot.bin 的写入位置和写入大小应当稍作调整以避免覆盖了 fs 的相关信息。*”，所以这就跟着手册定位到了原因，应该就是这一块有部分被直接覆盖，导致qemu识别硬盘不顺利。  

但是这个**fs元信息**是什么东西，为何这么重要，能导致qemu无法正常识别？  
查找资料，得知**fs元信息**就是在**mkfs**格式化磁盘/镜像时生成的信息，包括分块、创建超级块（文件系统基本参数）、初始化块位图、初始化根目录等，相当于**mkfs**构成了磁盘的**骨架**，而**元信息**就是**骨架的说明书**。这里被损坏后就无法正常识别。  

> 此外，还发现在0x4000、0x8000等位置（也就是16384=2^14=16KB）有标记，经过分析，应该是一个**簇**的起始标志，而根据手册可知共有16MB、32768个扇区，每扇区512B，那么**一个簇就对应32个扇区**。可是在定义时定义的-s是8啊？可能是两个文件不同？先mark住（）
> 但是不理解为什么手动写入与make生成的在接下来的几行有不同之处，猜测这是跟着内存变化的。
> ![](pictures/4.png)  
> ![](pictures/5.png)  

那么这就意味着找到不占用的写入方法，就能保留这些元信息了，这也就是手册中提到的思路。  
新创建一个空白的磁盘，查看其中的内容：  
```
dd if=/dev/zero of=empty.img bs=1024 count=16384
sudo mkfs.vfat -F 32 -s 8 empty.img
xxd empty.img > empty.hex
```

对比刚建立的文件系统与makedisk，发现似乎后者不同的地方是从0x60开始的，并且在0x40行就出现了不同的地方：  
![](pictures/6.png)  
并且发现在前面的簇起始区内容也不一样，三种各有各的样子：  
![](pictures/7.png)  
看不出来，那么继续，先把loader写入进来，观看前后变化。  

写入loader后，再转成新的hex文件，这次可以直接用diff来比较：`diff empty.hex empty2.hex`，可见前面起始区没有很大的变化，loader的确是写到了后面；然后有略微的变化，应该是文件系统自动进行的修改。  
![](pictures/8.png)  

那么观察**empty2.hex**与**makedisk.hex**，就可以看出来其是将boot写入到了0x5A之后的区域，就能够不影响fs元数据了。但是还有0x43后的4个字节也发生了变化，猜测这也是文件系统做的。  

那么接下来就研究一下如何让**dd**命令精确的在指定位置写入，查找资料，可以知道dd命令的几个参数用法：  
+ **bs=大小**：设置读写的块大小，单位是byte，这里需要精确操作，就设置为1
+ **seek=n**：在写入前跳过输出文件的n个块
+ **skip=n**：在读取时跳过n个块
+ **count=n**：指定复制的块数  
+ 还可以在bash下结合`$(())`算术拓展使用**十六进制数**

使用指令`dd if=boot.bin of=empty.img bs=1 seek=$((0x59)) count=512 conv=notrunc注意这里这个是错的要得是0x5A`，将boot的数据写进，这次应该差不多了。（事实上，看下面也知道并没有，出了一个小bug）  

直接挂载到qemu运行，却弹出窗口、报了错，显示`eadFailN`，这时也不能用diff分析，因为实际上make还增加了一个文本文件（后面任务要用到），会直接占满屏幕，这会继续肉眼分析前面这一部分即可。  

肉眼观察对比，发现是少偏移一个byte，但是明明要在**0x5A**之后写入，我放到**跳过0x59**不是正好吗？  
思考了一下，发现这就是经典的**索引**与**个数**的问题，从小到大，从年份到数组，总被这东西坑。的确我们是要**跳过0x59**，但这是**索引**，从0开始；而seek需要的是**数量**，从1开始，因此需要索引到0x59，那么数量就得是0x60。  

这样再来一遍，终于，成功了，手动！  
![](pictures/9.png)  

这时再参考**Makefile**中的做法：`	@dd if=$(BOOT_FILE) of=$@ bs=1 count=420 seek=90 conv=notrunc > /dev/null 2>&1`，这里大体是一样的，只是count数量不同，我的是512，但这里是420。  
但实际上，命令执行后的输出是
```
输入了 420+0 块记录
输出了 420+0 块记录
420 字节已复制，0.00678099 s，61.9 kB/s
```
这为什么就是420呢？发现是从**90字节**开始，加上**420字节**为**510字节**，再加上最后的**2字节**的签名，刚好凑够boot所需要的**512字节**。  

所以猜测这**420字节**刚好是整个**boot程序**的大小，难道真这么细节吗？并且最后的的签名为何在boot.asm中找到定义呢？  
这里就涉及到**boot.asm**文件了，在这里面大概看了一下，果然最后的`times   SizeOfBoot - ($-$$) db 0`填充命令中，sizeOfBoot是一个**512-90-2**，所以**boot.asm**会自动填充到这个值。  
至于最后的签名没有在boot中找到，其实是因为这在**mkfs.vfat**时候就创造好了。  

所以，这**420字节**就是第一扇区留给**boot**程序的实际空间。那么，为什么**fs元数据**就能在**90字节**前呢？因为实际上看刚创建好的文件系统，90字节前后明明都有相关数据，而且前面那次失败也说明少一字节都不行，这具体肯定有规定，所以到手册中给定的官方文档中看看。  

具体内容从文档**第7页**开始，大致介绍了**BPB结构**（可以理解成所谓的fs元数据？），其中就提到**0-35字节**是FAT文件**通用的**基本元数据定义区域，然后剩余区域则是**拓展区域**，大小和FAT格式有关，譬如**FAT12/16**最多到61字节，而对于**FAT32**则到了89字节，并且定义了最后的签名，如图：  
![](pictures/10.png)  

那么这下这个**90字节**就明白了，这时再看看十六进制文件在90字节左右翻译出的：  
```
  FAT32   ...w|.
".t.V.......^..2
.......This is n
ot a bootable di
sk.  Please inse
rt a bootable fl
oppy and..press 
any key to try a
gain ... .......
```
我猜测这里就是当一个磁盘的这个初始扇区没有boot，也就意味着这些字没有被覆盖，那么bios显示的时候就会把这些字符串显示出来，来起到提示作用。  

***综上，终于比较清晰的理解了如何自己手动写入、为什么会错、怎样才不会错，而不是仅仅调用make程序，分析对比hex也很~~有趣~~。***  

总之是把这第一个任务完成了！  

## 任务2

### 像boot一样寻找loader.bin

先看懂**boot.asm**是如何寻找loader.bin的，相关代码应该是**.find_loader**处的。  

一条一条看，把语法先搞懂，后面可以调试。  
+ **push**：把寄存器/立即数压入栈，实际上就是栈指针往下移，**SP=SP-2**，然后把数据写到**SS:SP**对应的内存处
+ **repe**：该指令是“*Repeat while Equal*”的意思，要与串指令一起用。这里的`repe cmpsb`是常见搭配，会先设置**CX**为计数器，然后比较**DS:SI**与**ES:DI**字节，如果相同，就继续比较下一个并**CX-1**，直到不同或者CX为0，此时**ZF=1**；如果不同就退出，设置**ZF=0**。所以这通常用于**比较两字符串是否相等**。
+ **jz**：与上面联动，意为*jump if Zero*，如果**ZF=1**则跳转，否则继续往下走。

大概道理懂了，但是**为什么loader要找loader.bin？**  
看了一下下面要求，大概意思应该是为下面找文件、并打印簇号做准备，因此要研究的大概就只有**find**这一部分。那么这样的话，还是得把boot.asm里面整体用gdb调试看一遍才好。  

但是在gdb里面layout asm(或者反汇编)得到的汇编与真实汇编出入极大，lab1简单，没什么差别；但lab2的这些逻辑更复杂了，机器码更多，反汇编出的更为杂乱，无法与**boot.asm**对应，太难受了。  

并且还总有很诡异的情况发生：当我直接看反汇编代码时，我甚至找不到**0x5a**，但是我步进时就能跳转到**0x5a**：  
![](pictures/11.png)  
![](pictures/12.png)  
仔细看一下，其实并没有发生什么**篡改**，但就是会少这么几行，继续往下还是：  
![](pictures/13.png)  
![](pictures/14.png)  
这里就是，ni之后，忽然就跳出来“隐藏行”了。  

就在这种逆天的环境下，小心翼翼的跳过这些陷阱，竟然还真找到**.find_loader**的位置了（但我其实都不知道为什么能走到这里）：  
![](pictures/15.png)  
其位置就在**0x7d86**!  

那么就试图把这一套找文件的逻辑搬到**loader**里面吧。  

想要创建一个变量名，但发现这次与之前不同，有两种写法，分别是`name dw xxx`和`label: dw xxx`，搜了一下，两者的效果基本一致，只是语义侧重点不同。  
并且变量格式还可以在后面更改。  

FAT32的文件名格式是基本的**8.3**格式，也就是8字节基础名+3字节拓展名，这也就解释了为何loader的名字是“LOADER(空格)(空格)BIN”了。  

不断从boot中**偷函数**过来，因为搜了一下，两个里面的调用并不能共享，所以每一个进程都要**copy**一下。。  
但是这样倒是**抄也要抄明白**，所以反而能够一点一点的分析各个部分，这里的东西都放到注释里了。  

在研究**EndOfBuffer**这里相关的目录项的时候，去手册中又了解了一下目录项，又亲自看了一下自己的hex文件，发现写入的loader文件本体是在**0xD000**处，也就是**104扇区**，属于数据区；而其**目录项**则是在**0xC000**处，也就是**96扇区**，刚好在**数据区**的开头。  
而**96+8=104**，所以可知这是一个目录项簇。  

现在一直从boot中**偷**来各种函数并试图理解，但我感觉似乎到后面的loader了，应该有的值可以变了？但越看脑子越乱，好糊涂。  

感觉对FAT32的理解还没有一个比较**全面**的认知，仅凭看了创造出来的十六进制文件只能是管中窥豹，还得系统的看一下介绍来构建起全面的认知才好。尽管学长在文档里已经给出了[微软的官方文档](https://academy.cba.mit.edu/classes/networking_communications/SD/FAT.pdf)，但是由于初学、并且英文水平太弱，还是找了一个[中文博客](https://zhuanlan.zhihu.com/p/718253579)来阅读，希望在这之后遇到问题再去官方文档，能够有所收获。  

这时再对hex文件进行分析，就能解决一些小问题了。  
+ **DBR**扇区，也就是第一个512B中的内容，其在**第6扇区**(0xC0)还有一个副本，也难怪当时看会有两处一模一样，但是由于boot是后来手动写入到DBR的，所以实际上看DBR副本并没有和原本一样。
+ **FAT1**区，从**第32扇区**开始（0x4000），每个FAT项为32位。每一项就对应后面内容的一个簇，前两项却是保留用的。所以从**第2个FAT项**开始，对应**数据区的第0个簇**。看所说的第0项与第1项一般都保留，所以固定，这吻合；第2项表示文件系统根目录，并且其中内容并**未超过1个簇**，所以是设为0x0FFFFFFF8，也可以解释为尽头。此外**微软**文档中再次体现了小端序列。
+ **内容区**起点，由前面可知，0-31是保留扇区，32往后是FAT区，有一主一备两个FAT表，占据32x2个扇区，因此内容区的起点是**96扇区**、**0簇**、**0xC000**。这里也解释了前面的问题，数据区之前的都没有**簇**这一概念。
+ **一一对应**一下，第2项FAT指向根目录，也就是文件系统的第0个簇（由于对应关系，实际上簇号是2），地址是0xC000，里面的确存放了一个LOADERBIN，再仔细研究可以得到其**起始簇号的高16位为0000**，**起始簇号的低16位为0300**，组合起来是**00000300**，但其实由于小端，是**00000003**，也就是数据区的**簇号为3的**，位置就是**0xC000+3x8x512=0xF000**；要访问这一文件，就到0xF000去。

这么一来，就可以继续去loader来完成了。  
+ **movzx**：移动，并且高位填充0
+ **mul**：只接受一个操作数，另一个隐式放在eax，并且根据实际位数，结果放在不同位置

**Packet**：陌生的概念，但是有实验指导中的提示。  

**两个.fail**：发现有两个**.fail**处理标签，这是为什么，不会冲突吗？查询得知，这是**NASM**与**GAS**中的**局部标签**。其作用域会局限在上一层全局标签，所以一个在**Init**全局标签下，一个在**ReadSector**全局标签下，就不会产生重定义错误。  

**带类型的内存写入指令**：发现有好多类似这句的写法：`mov     word [si + Packet_BufferPacketSize], SizeOfPacket`，十分令人疑惑，查询得知，这种写法是`mov type [addr] , data`，告诉编译器**指定内存地址**中要写入的**字节数**。

现在发现把除了读取簇数据之外的这一串看完，基本就明白了，继续照猫画虎补全就可以，不懂的地方先放一放，不至于影响。  
把头文件引用、MSG编辑等东西都一一对应添加好、做适当的修改。  

研究boot中的语句发现，```    mov ax, 0x0003 + int 10h```这一命令就能够清屏，原理是bios显示模式切换时会清空所有内容。  

发现在**boot**的Init中，将所有段的基址都设成了当前代码段cs，也因此发现其中的栈几乎都是基于代码段的。查询得知，这样做的原因是boot的**代码量很少**，而且进入boot时其他段都**未初始化**，就采取了这一**简化策略**。  
但是对于现在正在完成的**loader**以及后面的**实模式**，最好就要把这些段分开使用，根据GPT的建议，现在就可以把整个内存空间重构为平时**熟悉的那种内存布局**（下面是堆、上面是栈）。我去，我就说为啥一直迷迷糊糊的。  
具体的分配见代码。  

在实现**NextCluster**时，由于一开始没能理解boot中的算法，所以是字节从头写，过程中才逐渐明白这计算的道理。并且发现之所以用di作为副本的原因是**16位实模式下**特定的寻址十分严格，必须要有**bx、bp、si、di**中的一个才可以使用。  

此时基本写好了，但要**debug**，发现**boot**中的**found**标签地址为0x7db0，大跳是0x7dea。  

发现果然如手册中所说，**loader**中的汇编完全看不出来在干什么，于是重新在群里看了下同学的讨论，着手构建代码，用到了没见过的**ninja**，要先用`./config xxx`进行一系列的编译设置，然后再`ninja -c build/`进行编译，最后再用`sudo ninja -c build install`进行安装即可。安装完后在gdb中就能够看到正常的代码了。  
但是只能用vnc端口来监视，因为这个构建出来不支持gtk、sdl等，得在configure中就做好。。。  
所以使用vncviewer好了。  

这样就能够继续debug了，我的Init是在**0x904a6**上跳转（但后来发现随着更改代码，这地址会变）。  

接下来就分析出现的bug，是运行时只显示一个光标在跳动，连我的loader信息都不显示，debug单步运行，发现会显示loader信息，但是在我调用message显示后变成这样的，这说明**在调用showmessage**相关时出现了错误，看来不能直接用现成的了……  
所以先放弃显示相关的，把有关message的都注释掉，再次运行，这次果然没有这个问题了，那么最后是停留在正常情况下的死循环还是在失败的死循环内？  

再次单步调试，每当要跳转前就打下断点，如果能回来就说明没问题，否则就陷在跳转的相关函数内了，这次的死循环就被我定位在了这里：  
![](pictures/firstdeadloop.png)  
是刚init后，在load_dir_cluster中获取根目录的簇，师出不利。  

**readcluster**位置在0x90526。  

调试发现，似乎是因为**获取簇号**这一块出现了问题，eax本应是根目录的簇号，也就是2，但是显示为0。  
分析发现，原因是我想多了，BPB相关内容本来就是固定的地址的，所以不用再加上一个段。  

再往深挖，那几个全局变量是定义在**start**段内的，属于是**cs**段内的范畴。但是后面使用时候没有加前缀，默认用**ds**访问，导致错乱。  

但是继续往下，还是会卡死，再次定位到是执行**int 13h**中断时卡死的，应该是因为我并没有配置让**ds:si**指向那个结构体；并且栈的细节也不太对。  

重新更正，再次运行，发现依然会卡死，检查，发现有很奇怪的地方：我令**si=sp**，**ds=ss**，却发现直接用`x/16x ($ss*16+$si)`、`x/16x ($ss*16+$sp)`以及直接读取地址，显示的地址都不一样，问GPT，分析可能是gdb中有点问题，但实际写入的没有问题。  
可是在写完结构体后检查地址上的字节，依然没有写入，再次研究半天，发现可能是栈的位置写到了另一个只读区域（现在是在0xD000为基址）：  
![](pictures/memory.png)  
把栈放到与loader一样的0x9000基址上，再次运行。  

这次那个bug问题仍在，但是检查实际地址，确实是写了进去：  
![](pictures/writesuccess.png)  

这下继续，终于没有再死循环，属于是成功的把一簇读到了**es:bx**处。  

这次继续运行，竟然难得的直接显示我预设的成功提示：  
![](pictures/success.png)  
这里是真没有想到，这组成了一个**S\*\*T**，原本想的是Success：hit  
![](pictures/code.png)  
立刻改成Y，唉唉。  

可是我还有疑惑，这里比较匹配名字是基于**DS:SI**与**ES:DI**，但我的FileName算是定义在CS段内的，两个明明也不一样，本来都想好出bug的，竟然成功，十分不解。  
仔细看，这里没有改动FileName，而是相当于直接使用硬编码的文件名，然后将其`mov si, FileName`，才发觉这不太对，这相当于把一个地址写到了寄存器里？  
查了一下，像这里直接使用变量名，是直接获取此变量的offset，但是是基于CS的，但是比较时候是基于DS，两者**不相等**，按理来说是不可能成功的。  

再看，却发现DS不知道在哪里从原来的**0x0050**变成了与CS相等的**0x9000**，两个BUG竟然顺利抵消掉。再定位这是在哪里发生的：  
+ 在初始化时，为了最大化利用可用的内存空间，**复用**了loader的**CS**与**SS**，都为0x9000
+ 在**ReadSector**中，为了让**ds:si**指向栈上的packet结构体，将**DS**设为了**SS**，并以为pushad、popad会恢复段基址寄存器
+ 实际上**pushad、popad并不会存储/恢复段基址寄存器**
+ 所以实际上到后面就奇妙的实现了**DS=CS**了。  

进行修改后，这次终于是较好的实现了预期功能：**查找loader.bin**。  

### 将簇号打印至终端

打印至终端，肯定还是有相关的**中断**能够把特定数据返回到终端来显示，但是看着一整页的中断，感觉一个一个找肯定不行，只好请教GPT老师了……  

问了一下，这里的**终端**其实也就是屏幕，那么就又要用**int10**来实现输出字符。给出的**WriteChar**是比中断更加底层的显示操作，直接操作**显存**，但是只适合输出固定的东西，像簇号这种变量就不好实现了。  

于是把lab1实现的那几个东西照搬过来，然后编写一些新的辅助函数。  

关于把簇号的数值转换为**ASCLL**字符，这里确实还得一番编排。  
我的结构是：  
+ 调用**DispClusterNum**函数
+ 把簇号**拆成4个字节**，按字节显示
+ 把每个字节**拆成高4位与低4位**，按一个十六进制数来显示
+ 把**十六进制数与9**相比，若小于等于，则加48，否则加65显示（ASCLL规律）  

再经过一些debug，终于很顺利的实现了打印簇号的预期功能：  
![](pictures/clusternum.png)  
当然还略显粗糙，但基本成型。  

忽然发现不对，这样打印出来的是根目录的簇号，而非真正文件的簇号！  
需要从目录项中把文件的簇号提取出来。  
修改后的显示结果：  
![](pictures/clusternum2.png)  

### 将aA1.txt簇号打印至终端

首先创建aA1.txt文件：`dd if=/dev/zero of=aA1.txt bs=1024 count=40`，这样就能创建一个40kb的文件。  

将其写入**outchain.img**后，还更改了loader文件中的**FileName**变量，用上面打印簇号的逻辑跑了一下，竟然还是0x2簇，检查了一下发现是显示的根目录簇，而不是目标文件簇，进行修改后正常，那么就得研究如何能够显示簇链。  

显示簇链无非是**显示簇号**与**沿簇链寻找下一簇**的组合罢了，拼起来就实现了打印该文件的所有簇号（反而感觉这个并不是很难？）  
![](pictures/clusterchain.png)  

### 填充aA1.txt并打印其前10个字符

到这里，先按照提示，随机生成一个40KB的aA1.txt文件，将其写入即可；  

显示前10个字符比较容易，把第一个簇读取下来，然后让es:bx指向其开头，再用中断就好了。  

跑出来的结果十分诡异：  
![](pictures/ugly10char.png)  

怀疑是因为随机生成的内容过于随机，到了非标准的ASCLL区域，不能正常显示，所以的确得限制ASCLL的范围。  

给出的这些提示果然有用，虽然还不熟悉，但问GPT并且详解后就懂得了：  
`dd if=/dev/urandom bs=1 count=81920 2>/dev/null | tr -cd 'a-zA-Z0-9' | head -c 40960 > aA1.txt`  
是一个比较复杂的管道：首先把**urandom**作为输入，读取81920个字节（也就是80KiB），并把错误输出重定向到**null**，这样就不会干扰管道，然后把输出字节经过**tr**来过滤，只接受数字、字母，再通过管道到**head**，限制只读40960也就是40KB，最后输出给**aA1.txt**。  

重新弄，这次却簇号有点少：  
![](pictures/less10char.png)  
原因可能是80kb的筛掉后不一定够40kb，再次修改，得到一个最佳的方法：  
```
while true; do
  dd if=/dev/urandom bs=1 count=81920 2>/dev/null | tr -cd 'a-zA-Z0-9'
done | head -c 40960 > aA1.txt
```
这样就会不断的无限循环生成，然后通过管道传给**head**命令，凑够40kb后就会关闭管道，循环中止。  
但是实践发现其不会自动终止，但文件的确没有错。  

这样再来，就没有问题了：  
![](pictures/true10char.png)  

### 改变512-515字节为GOOD

这里应该就得找一下**写磁盘**的相关操作了，因为修改后终归是得把新内容写回去。  

果然“三步之内必有解药”：  
![](pictures/int13.png)  

看了一下，果然和读取是很类似的：  
+ AH置为**43h**
+ AL为写标志，在**早期版本**按位控制，**现代版本**是整个AL来区分，并且为**00h、01h**是无验证写入，为**02h**是有验证写入
+ DL为**驱动号**
+ DS:SI是**指向packet**位置（和read里面一样的）
+ 返回值CF为0则为成功写入
+ AH被重设为00h（若成功）

> 验证写入：在把数据写入目标扇区后再读回来，对比是否一致  

那么就基于**readcluster**与**readsector**来完成就好

完成后，试着运行一下，发现不对劲，怎么我前10个字符变成了GOOD……？  
![](pictures/wrongGOOD.png)  
看来还是有点失败，并且我的img由于反复写入，我发现里面十分的乱，原因应该是每当写入新的同名文件，其会把FAT中重新更新，但簇内的内容并不会覆盖。  

进行debug，可以确定是写入出了错，因为查看修改后的内容，是修改成功了的。  
那么问题就出在具体的**writecluster**。  
这次很快看到了不对劲之处：初始时bx为0，我加了512，那么最后就是从512开始写的……  

修改后，重新创建一个txt，将其写入，然后再次运行。  
前面的都一切正常：  
![](pictures/front.png)  
新修改的GOOD也在其中：  
![](pictures/GOOD.png)  

如此，终于完成了2。  

## 任务3

首先试着运行一下，果然显示“This is not a bootable disk”，还是得转成hex看看。  

来到**FAT1**所在的**0x4000**，果然是一团乱麻，但看到手册中写到的“第一张FAT表”，那么说明作为其备份的**第二张FAT**并没有被污染，位于**0x8000**，看了一下，果然没有，但是看着是相互交织，簇链十分的复杂。  

另外在默认的root簇上的FAT表为0，并且到地址c000处也为乱码。  

我倒是现在除了这簇链之外，是没有一点思路，所以打算从簇链入手。  

### 从簇链入手

由于FAT中的这些过于复杂，相互引用，肉眼不好看，但是鉴于前面的经验，这是一个链表，从里面随便找一个簇往后找，一定能找到尽头，再加上相关的标志位来区分是否访问过，如此往复，最后把簇链拼起来，应该至少能找到loader、彩蛋程序的对应簇了吧。  

思路就是如此，下面就是编写这个c了，目测得用上两个大数组，一个存整个FAT2，另一个作为是否可访问的标签，然后得用上数据结构的链表（还得复习……）  

第一个问题是如何把这个FAT表进行隔断，变成能直接放到C语言中的数组，我的想法是用python，这玩意效率更高一点（应该），但其实我两个都不咋熟练，干脆当个c仙人，按照手册中说的全程手搓吧。  

### 有没有操作系统

上面的簇链还没写几行，忽然对这个磁盘**到底是什么**产生疑惑。  

现在所知的这些都是**boot-loader-……**的操作系统结构，但是根据目标来看，这个损坏了的镜像似乎和u盘、硬盘一样，本身并没有操作系统，而是单纯的存放文件。  

所以对于没有操作系统、纯数据的盘，其结构又有何不同？我的猜想是其只是没有boot引导。  
问了一下GPT，果然，那就没有什么问题了。  

另外，还看了一下文档中的root相关的：  
![](pictures/16.png)  
root的第一簇存放在BPB结构中。  
![](pictures/17.png)  
并且这一结构还位于FAT32的拓展BPB区域，也就是**第44-47字节偏移处**。  

在hex文件中直接查看：   
![](pictures/18.png)  
那么就是**4e bf 30 19**？  
这大的有点不对劲，看看前面步骤得到的镜像中是多少：  
![](pictures/19.png)  
怎么也不对劲：**4e 2d be 16**？  

再次查看，发现了出错的地方：手册是十进制。  
十六进制是**2c**的地方。  

那么再看：  
![](pictures/20.png)  
这次就正常了，为**0x00 00 0e 9d**。  

**0xe9d**是十进制的**3741**，而FAT中，地址每增加**0x100**，就会多4x16=64个FAT项。  
这样用3741/64=58，并且余29，也就是3A，余1D。  
余的这1D除4为7余1。  
那么root簇就在**0x8000+0x3A00+0x0070+0x0004**处，也就是**0xBA74**。  

定位到FAT中对应簇，是没有问题的：  
![](pictures/21.png)  
说明这个root只有一簇。  

那么就找到其位置：  
**起始与两张FAT表**的扇区为0-95共96，**每簇有8扇区**，第**3741**簇的有效值只有**3739**，也就是扇区为**29912**，加上初始偏移，那么就是**30008**扇区。  

其对应的地址：  
30008扇区，一个扇区**512**字节，也就是**0x200**，那么对应的地址就是**30008x0x200**=**0xEA7000**。  
找到这里来：  
![](pictures/22.png)  

看得出来，这是一个名为“easter.egg”的程序（果然是彩蛋），并且其起始簇的高16位为**9187**，低16位为**a8f6**，整合起来就是**0x87 91 f6 a8**。这显然是一个很离谱的簇号，这应该就是所谓的  
> 根目录文件夹中有一个彩蛋，但是它的起始簇号被未知数据覆盖，但万幸的是其余部分均未损坏  

那么现在整理一下已知的信息：  
+ FAT1损坏，但是其备份FAT2完好
+ BPB完好，能够找到root的起始簇
+ root只有一簇，并且其中只有一个彩蛋文件的目录项，说明其就是一个单纯的磁盘
+ 彩蛋文件的目录项中起始簇号被污染了

那么我的思路清晰了：root中只有一个文件，说明我如果遍历整个FAT表，梳理清其中的关系，那么到最后就只有两条簇链，一条是root的，一条是彩蛋文件的，把彩蛋文件的起始簇号放到目录项中，就可以恢复目录项了！  

所以抓簇链是对的，那么继续切回去完成我的c代码罢。  

### 抓簇链代码

基本完成后，编译运行……  

果然没有什么结果，还是得debug。  

先检查读簇号是否读的对：  
![](pictures/23.png)  
将这个结果与hex中对比，发现是没有问题的。  

……  

debug了一会儿，略过吧，忽然不想回忆。。。  

现在运行已经不会出错了，并且能够正常输出，但是不符合预期：  
![](pictures/24.png)  

我猜显示的这个就是彩蛋的起始文件了，但是为什么没有显示root的簇链呢？  

找了一圈，发现是自己眼瞎——把0x0FFFFFF8写成了0xFFFFFFF8……  
这就又是**2个bug组合成一个算是正常的程序**。  

细节不想再研究了，现在就已经没有问题了：  
![](pictures/25.png)  
正好一个是root（**e9d**），一个是彩蛋（**4d6**）。  
并且与我的推测是相符的！  

### 修复回去

彩蛋的起始簇是**0x000004d6**，写成小端是**d6 04 00 00**，  
那么目录项中的**高16位**应为**0000**，**低16位**为**d604**，把这个写进去就没问题了（大概）  

那么就用之前的**dd**命令，像写入**boot**一样写入这个镜像。  
（先创建一个副本，防止翻车，因为这有点大）  

**先试试把东西读出来**：  
```
dd if=broken_disk_copy.img of=111.bin bs=1 skip=15364144 count=16
xxd 111.bin 111.hex
```
尝试读取目录项有关起始簇的那一整行，然后转换得到hex文件，查看，发现的确读到了：  
![](pictures/26.png)  

那么再尝试写，但是怎么写呢，把hex文件能转换为bin文件吗？  
查了一下，发现**xxd -r**就能倒着转换回去，还挺方便。  

那么就修改111.hex之后  
```
xxd -r 111.hex 111.bin
dd if=111.bin of=broken_disk_copy.img bs=1 seek=15364144 count=16 conv=notrunc
```
就应该正常写入了。  

那么就基本完成了，最后的问题是FAT1损坏了、那么FAT2在挂载时会自动识别吗？  

尝试着挂载了一下，果然是不太行，会报错“can't read superblock on /dev/loop0”  
看来还得把FAT2给写到FAT1上。  
看有专门的修复工具，但应该就是一个复制粘贴的操作，干脆也手动完成算了。  

又是一段尝试，写出了这样的：  
`dd if=broken_disk_copy.img of=broken_disk_copy.img bs=1 skip=32768 seek=16384 count=16384 conv=notrunc `，但是却始终不能运行。  
经过GPT的分析，原来是对于if与of为同一文件，**只能从前读，往后写**，而不能反过来（读后面，写前面）。  
方法就是加一个中间缓冲文件。  
（上面这个操作也使得我的img被截断成16384字节，幸好提前备份了）  
```
dd if=broken_disk_copy.img of=111.bin bs=1 skip=32768 count=16384
dd if=111.bin of=broken_disk_copy.img bs=1 seek=16384 count=16384 conv=notrunc
```

### 终于！

![](pictures/27.png)  	
无需多言了 : )  

### 彩蛋的一些观察

彩蛋的这个输出，在群里已经有同学剧透过了，但是学长说**彩蛋没有更新，被当黑户了**，说明这个程序还会根据某些信息来进行不同区分，从而展现“完全版”以及黑户版？  

我首先想到可能程序执行时会**读取时间**？彩蛋是老的，去年的试点班肯定能执行这个彩蛋，但今年试点班执行不了这个彩蛋，可能就是时间不一样。  
于是问GPT：**执行一个可执行程序时，其会读取我的系统时间吗**，其回答是看具体有没有相关的系统调用，并给了我一个工具：**strace**，其会追踪一个程序执行时所申请的系统调用。  

试着执行了一下：`starce ./easter.egg`，输出为：  
![](pictures/28.png)  

说实话，抛开半懂的，唯一能看出来东西的就只有两句：  
```
...
readlinkat(AT_FDCWD, "/proc/self/exe", "/home/liuzt/Documents/NPUOS_Assi"..., 4096) = 63
...
read(3, "origin\tgit@gitee.com:pilot_2025/"..., 1024) = 222
...
```
当然是继续问GPT了。  

问出来得到，第一句是系统调用**readlinkat**，具体作用是反推得到当前执行这个程序的路径，第二句是系统调用**read**，打开一个文件，读取到了第二个参数中的那些数据。并且其还帮我推测出这属于**.git/config**文件中的  
```
[remote "origin"]
	url = git@gitee.com:pilot_2025/2023302661-OS.git
	fetch = +refs/heads/*:refs/remotes/origin/*
```
这一字段，所以其不是获取我的**系统时间**，而是看我的仓库信息和其预设是否相符，**相符则正常彩蛋，不符则提示“外星人”**？  

结合助教说的没有更新彩蛋，那么其可能比较的是**2024年试点班**的相关数据了。  

在qq群里找了许久，没有找到2024年仓库的相关信息。  
忽然想到，为什么不试试直接**到gitee把2025改成2024**？  
果然能够到达24年的仓库中：  
![](pictures/29.png)  

将其clone到另一个文件中，查看其config信息，那个熟悉的字段：  
```
[remote "origin"]
	url = git@gitee.com:pilot_2024/os-labs.git
	fetch = +refs/heads/*:refs/remotes/origin/*
```
把彩蛋复制到这个仓库中，运行，无果，却还是拒之门外。  

仔细对比该字段与我的仓库中该字段，发现似乎是因为这不是我的“自建分支”，使用vim按照格式修改为我的学号：  
`url = git@gitee.com:pilot_2024/os-labs.git`  
依然报错。  

再阅读一下24年的手册，基本除了年份，没有什么变化；  
但是看到这里面面有一个神秘小网站：`git remote set-url --push origin https://gitee.com/NWPU-OS-EXTRA2024/2022306666-OS.git`，我试图进入`https://gitee.com/NWPU-OS-EXTRA2024`，提示权限不足，访问受限（事实上，今年的手册也有**NWPU-OS-EXTRA2025**，但是如果进去的话会是404，应该是直接不存在）  

提到**权限**，忽然对味了：也许所谓权限，就是去年助教根据试点班的学长们的学号分配仓库后相关配好的！  

那么到qq群里，~~随机挑选一名学号2022开头的幸运学长~~，借ta的学号一用（**也多亏老师是把每一届试点班放在一个群，而不是每年建一个新群**），运行：  
![](pictures/30.png)  
以及在当前目录生成的学长照片一张～（原来群里的bro剧透到底了）  

**wow ：）**  
~~这算是野路子破解法么~~  

重新整理一下，也就是说彩蛋程序执行时，会获取当前路径，然后转到**.git/config**文件中读取**[remote "origin"]**字段，将其与硬编码的2024年试点班学生学号进行比较，从而展现彩蛋。  

***

# LAB3

## 制作分区硬盘

上来首先大体阅读一下整体实验文档，然后开始写。  

文档推荐所有工作都直接用**make xxx**里面的标准方式来，但是步骤1很显然是要自己来手动做一做。根据上一节的经验，那就依然是自动构建的放在build，自己构建的放到上一级，这样能够方便的对比。  

首先**自动构建**，在最后报了错：  
```
2025-10-17 01:32:31.471 [DONE] ld kernel.bin
2025-10-17 01:32:31.476 [PROC] build image disk.img[sudo] liuzt 的密码：
mount: /tmp/tmp.6QyvrZT800: /dev/loop0p2 is not a block device; try "-o loop".
       dmesg(1) may have more information after failed mount system call.
umount: /tmp/tmp.6QyvrZT800: not mounted.
mount: /tmp/tmp.6QyvrZT800: /dev/loop0p4 is not a block device; try "-o loop".
       dmesg(1) may have more information after failed mount system call.
umount: /tmp/tmp.6QyvrZT800: not mounted.
2025-10-17 01:32:36.058 [DONE] build image disk.img
```

看样子错的还不少。  
那还是手动试试吧。  

按照顺序来，到了用**cfdisk**来分区，发现这个界面和装系统时候那个界面好像。按照**part.sfdisk**文件来分。   

分完后，尝试进行初始化，但是发现对p1、p2、p4的初始化都失败了。  

这个情况在群里有同学提出，助教说是回环设备爆掉了，似乎是重启就能好？  

重启后，再次尝试对p1、p2、p4的初始化，这次p1依旧不行，但是2和4都只有警告、没有报错了。  
并且从注释中可知，我们只用到了2和4，剩下都是图一乐，就不用再深究了，继续往下就好。  

这时候看分区情况：  
![](pictures/31.png)  

然后编译mbr程序，编译中报错找不到**layout.inc**，但是有一个**layout.inc.in**，看手册中的有关部分，原来是头文件还需另外配置。折腾一番，写成`nasm -I build/include/ -I include/bootloader/ src/boot/mbr.asm -o mbr.bin`就能自己搞出mbr.bin了。  

可知要把mbr写进磁盘的第0个扇区，就在0-445，这个好，不用怕像boot一样写到错误的地方了。  

写入后挂载、运行，结果与预期一致：  
![](pictures/32.png)  

## 修改MBR程序

接下来就得继续再改MBR程序，来满足不同需求。那么我的方针依旧是先看懂，再修改。  

+ **cld**+**rep movsb**组合：**cld**是用来清除方向标志位，让字符串操作按地址从低到高来处理，而**rep movsb**则是根据CX的值不断把[DS:SI]中的字节搬到[ES:DI]，因此**组合起来**后的语义就变为了**复制CX个字节，从DS:SI到ES:DI**处。  

这次的框架比较简单，所以很快就能着手修改。  

### 显示四大分区有关数据

这里出的大bug是由于内存分配不合理，导致中断向量表被覆盖。  
最后参考[这篇文章](https://blog.csdn.net/m0_51174487/article/details/127307181)来把理清了内存布局。  

效果如下：  
![](pictures/33.png)  

### 搜索并启动第一个活动主分区

这里应该要用到分区状态和分区所占扇区数目了。  

这应该不难，但是似乎我写的有点多，编译时候显示`src/boot/mbr.asm:308: error: TIMES value -33 is negative`，得简化一下……  

简化后又带来许多bug，于是再debug半天。  

最后终于是能找到了，但因为没有烧录boot，所以显示的是没有bootable。  
![](pictures/34.png)  

于是就又要弄烧录镜像，实在不想手动了，就按照手册用make命令。但是又开始报错了，挂载不上，那么只好重启了……  

结果发现重启也没用。依旧挂不上，真就只好自己来。  
那就先空着吧？因为能到这里，说明就只差一个boot了。  

不行，还得搞一下。成了，见我的小文档：  
![](pictures/35.png)  

### 让boot.asm打印所在主分区序号

调试一下，发现我在MBR中把主分区序号放到了cx，并且跳过来够cx的值并没有变，所以直接打印cx中的值就行（当然要按照ASCLL码来）  
![](pictures/36.png)  

然后是修改分区表，我是采用的修改**part.sfdisk**文件，把**disk.img2**改为`disk.img2: start=4096, size=2880, type=4`，然后再次尝试，解决一个bug（因为发现cx实际上不是分区号，而是读取的扇区数，通过一个push和pop就可以解决）后，就能正常了：  
![](pictures/37.png)  

## 修改boot.asm程序完成内核启动

这里其实在完成上述过程中就已经注意到要补全驱动号与起始LBA，将其补全即可：  
![](pictures/38.png)  

这里有一点，就是boot在我实现上面的打印主分区序号的情况下是写不下赋值语句的，所以只好将其注释掉……  

然后接下来就只要用GDB观察实现过程即可。  
试着结合loader的代码，用GDB调试，发现这次的loader是格外的复杂，大概看到加载进实模式后就已经晕头转向了。哎，任重道远啊，继续先往下吧……  

## 修改kprintf.asm

### 明白要干什么

一开始完全不知道究竟该干什么，阅读手册后也是迷迷糊糊。  

但还是硬着头开始编。  

一些踩的坑：  
+ 栈会以**4字节**对齐
+ inc可以替代add 1来自增
+ 栈传参要谨慎使用pushad和popad，有把自己绕晕的风险

### NWPU显示

经过痛苦的debug，调通了：  
![](pictures/39.png)  

### 显示十四行诗

那么接着往下看，发现显示十四行诗也太慢了，单步执行就到猴年马月了，断点也不好打，所以干脆直接跑：  
![](pictures/40.png)  

基本正常，可是有一大坨报错，这咋回事？  
发现是在**cstart**中有一个**unreachable()**，F12全局搜索不到定义，奇了怪了。那么全局搜索文本，发现这原来不是函数，而是一个宏定义：  
![](pictures/41png)  

原型是`#define panic(msg) _panic(__FILE__, __FUNCTION__, MH_STRINGIFY(__LINE__), msg)`，所以这其实是一个正常报错。  

### 先跳过%s，先看cmatrix的那个接口

发现没啥特别的，直接调用就行，就只需要多include一下。  

好玩，牛b！  
![](pictures/42.png)  

### 完成%s的配置

这里需要对结构体内部对齐、栈对齐有深刻认识，但我并不熟悉，写了一会发现一直出不来，只好再debug一下。  

最后通过`x/32xd $esp`命令来检查栈，才发现其和我想象中不一样：  
![](pictures/43.png)  
具体细节放代码里。  

然后再写，就能行了：  
![](pictures/44.png)  

***

# Lab4 中断

## 验证时钟中断的发生

### 编译运行基准代码

首先依旧是要把**make-image.sh**给改成适配本机的，也就是要给sfidsk前加上**sudo**。  

然后就用**make run**观察即可：  
![](pictures/45.png)  
可见最上面的黄字与前面的panic一样，似乎是自定义的一些提示；下面则就是源源不断的井号与ABC。  

到源码中看看这是什么，定位到**keyboard.c**中的`init_keyboard()`函数，涉及到`unimplemented`与`enable_irq`。其中前者是一个超级复杂的宏：`MH_EXPAND(UNIMPLEMENTED_IMPL(MH_EXPAND(MH_NARG(__VA_ARGS__)), __VA_ARGS__))`，而后者则是一个比较完整的函数调用。  

### 修改时钟中断

首先再看看代码结构，发现其中有很多**outb**、**outsb**等函数调用，这是看不明白的，所以跳转至其定义，发现就是一个同名的汇编调用。  
这个调用专门用来与外设通信的。  

在**time.c**中的**init syscall**函数中，有`put_irq_handler`与`enable_irq`的组合，其作用是把时钟中断与其handler绑定，然后再启用中断。  

而`clock_handler`就是handler，其负责打印“\#”，我们的目的就是将其改写成为**ik**的形式。  
至于这个k，同步对比发现用**SYSTEM_TICK**比较合适。  
本以为这里的**kprintf**函数还是lab3中手动实现的简陋版本，但看了一下，原来已经是功能完备，能直接用各种格式化字符串的了。所以直接写`kprintf("i%d.", SYSTEM_TICK);`就好：  
![](pictures/46.png)  

### 向进程传递参数

但接下来的一步就sb的很了。  
懵懵懂懂，只知道按照规矩压栈，但压入后一直显示#GP报错，还不知道怎么调试。调调调半天p用没有，问ai也唐的一批，什么东西都弄不出来。  

连着两天了，这个东西是怎么写都修不好：  
![](pictures/47.png)  

换了一轮又一轮ai，让其帮助我修，结果修不好。  

逆天了。把函数中的**todo**注释掉，立刻就好了，这几天试的各种方法，统统都过关。但是这不应该是显示这里todo的信息吗？为啥会抽到interrupt中去？草了。  
![](pictures/48.png)  

没得说，观察，发现基本符合预期：A为红色，B为绿色，C为蓝色，第一个为标识符，表明来自哪一个进程，第二个为进程中自带的i的值，第三个为进程id；另外可见普通打印会继承上一次的颜色等。  

## 修改时钟中断触发调度机制

观察，这里说的调度应该是在**timer.c**中的这一段：  
```
void clock_handler(int irq) {
  kprintf("i%d.", SYSTEM_TICK);
  ++p_proc_ready;
  sys_tick_incr();
  if (p_proc_ready >= pcb_table + NR_PCBS) {
    p_proc_ready = pcb_table;
  }
}
```
其作用是，每次进入中断时，先打印信息，然后转换为下一个当前正在执行的用户进程，使tick自增，如果当前用户态进程超出总共的，那么就切换为第一个。  

所以这里要想让某个进程饥饿，那么就让分配变得不合理，比如每进入100次中断，只让进程C执行一两次，剩下的都在A和B之间分配，增加一个逻辑判断即可：  
![](pictures/49.png)  

从图片中可见，的确是限制了C进程，如果把限制范围调的很大，那么就更加饥饿了。  

## 修改硬件时钟频率

文档好难看……不知道是不是因为英语不好。  
但好在之前玩过STM32，这些东西的基本原理都是通的，把10改成1000就好。  

![](pictures/50.png)  

这里特征就是首先的刷新极快，然后可能会出现一些不是三种颜色的颜色，猜测可能是上一个中断还没有把颜色设置好，下一个就已经再改了，并肯呢个出现一个`set_color`的报错。  

## 实现键盘中断

这里就必须要跟着书来做了，所以还得对齐书中一些函数与实验框架的函数。此外还要在必要的地方增加定义。  

![](pictures/51.png)  

再继续，发现这些码基本与键盘排列是很相关的。  

还发现这里与书中的框架还是有很大的不同。  

发现这里的getch是不能够加上中断控制（打开、关闭的），否则就会报GP.  

但是最后的彩蛋也是十分牛逼。  
![](pictures/54.png)  

***

# Lab5 页表与系统调用

## 熟悉ELF文件格式

### 打印内核文件ELF信息

说真的，到现在我对这所谓**ELF**文件的认识为0，所以先看格式文档是必要的。  

大致看了手册中的简介，可知：  
1. **ELF文件**不是说其后缀为.elf这样，其本身就是一个**二进制文件**.bin，只不过是二进制文件的一种特别形式，所以按照手册中说的`file kernel.bin | tr ',' '\n'`来查看的话，就会显示如下的信息:  
	![](pictures/55.png)  
2. ELF文件是针对于**代码段**只读、**数据段**可读写、两个段位于不同内存地址等的需求的，所以就要构建一个数据结构来存储这些信息，这就是ELF文件格式  

但是往下也没有更详细的介绍了，只有去读文档、参考loaderkernel.c、elf.h来搞。  

结果`man elf`一打开就绷不住了：纯英文、总共1272行……太痛苦了，先逃避。  
现在只好到网上找一些中文资料。  

### 了解ELF

**ELF**格式更多是一种标准（Executable and Linkable Format）。  

ELF格式文件包括4个部分，前三个都是**表**，剩下的位置都是**节**，我们最终落实的也就是**节**：  
+ **ELF头表(head table)**：位置固定，在文件开头，给出一些基本的信息，可以凭此找到其他信息。
+ **程序头表(program head table)**：找到同类的节的聚集地
+ **节头表(section head table)**：细致的找到每个节的地址

其中，在**可执行文件**中，程序头表一定存在，节头表不一定存在，因为执行时按顺序进行，是把代码、数据等分开，按照权限分成几个大段即可执行，**执行操作用到的是程序头表**；  
而在**可链接文件**中，节头表一定存在，而程序头表不一定存在，因为我们写程序时会把各类符号零零散散，链接时需要用到原始的这些零散数据，因此是节头表，**链接操作用到的是节头表**。  

其具体内容则是比较详细、比较多，知道其是什么作用就行。  

### 修改loaderkernel打印信息

![](pictures/56.png)  

+ 说明加载的内核文件是ELF文件：magic字段为“xx ELF”，是ELF文件的特点
+ 指令集为i386：这一信息对应e_machine处，字段为3，查表可知为EM_386，也就是i386
+ 加载程序段的个数：这一信息对应e_phnum，表示程序头表中有几条信息（几个段），这里字段是5，也就一共5个
+ 加载各段的起始地址、加载长度、执行标志，这里就得更进一步，去读程序头表了。

![](pictures/57.png)  

继续写，把上卖弄有一些小bug清理、注释掉不需要的信息。  

![](pictures/58.png)  

这一次就展现了完全的程序头表信息。  
值得注意的是，这里的最后一段(segment 4)的起始地址与第一段(segment 0)是一样的，大小为0，十分诡异。  

并且与**readelf**的输出相比，其在内存上的位置也不对。  

再次复查，原因在于在内存上的位置使用了错误的计算方法，这里应该用节欧体自带的p_vaddr来获取，而我用了偏移+ELFADDR的算法，一个是加载的实际位置，一个是刚加载好的物理地址。  

![](pictures/59.png)  

3224367104 -> 0xc02ff000，这回是正确的。  

## 加载用户程序

### 补全kmain

这里真的是糊涂至极，根本看不懂要我干啥。  

只好结合AI，反复看反复看才了解我到底要做些什么。  

![](pictures/60.png)  

这里出现了段错误，仔细看，是说eip的设置，回顾一下，的确是这个寄存器并没有被设置。  

eip是程序存放的地方，结合具体语义，就是elf文件的entry项了。  

![](pictures/61.png)  

这次，应该就是ok了！  

### 替换可执行程序

把原来的两个进程名换成**TESTPID BIN**和**TESTKEY BIN**再运行即可，程序运行时，会每隔一段时间打印出当前进程PID，同时会打印键盘按下的字母。  
![](pictures/62.png)  

## 实现系统调用

### 实现delay_ticks方法

首先要根据手册，先看看其声明中对功能的描述。  

延时给定的tick数，结束后返回实际延时ticks-预期延时ticks。  

第一个问题是输出全为红色的im0，格外阴森：  
![](pictures/63.png)  

检查发现是没有好好看pcb中的定义，priority的值我理解成为“优先级”了，为了让三个进程完全平行，都给设成了0，结果就只有pid为0的进程独享。  

接下来就将其改成1,1,1，第一次运行起初是好的，后面就似乎由于某些误差，导致剩下两个进程相继挂掉，然后再运行一次，发现一开始就会触发assert而挂掉，似乎这个参数还是不太稳定。  

![](pictures/64.png)  

再次分析，从用户态下的main可知，三个进程的要求的delay分别是1000,2000,3000个ticks，设成111的话，每有一个tick，就会在上下文进行切换，这应该就是比较好的了。  

那么换一种角度，不会触发断言的范围是998-1002(1998-2002等)，我原本的逻辑是只有相等时才会退出delay，这相当于自断一臂，只给自己留了1000-1002，那我现在让**目标延时ticks-当前延时ticks**<=2时就往下，或许宽裕度会大一点？  

似乎这次成功了，一边写笔记一边让其继续跑着，~~至少现在还没有~~话还没说完，一看又断言失败了。不过这次坚持的时间很久很久，可是问题显然还没有最终解决，这次只是坚持的更久；再次运行，也依然有几次刚进来就有几个进程挂掉。  

### 睡眠机制

目前来看，原因还是因为现在的版本进程调度已经是理论极限（每个只分一片），但中间的误差还是无法避免。  
了解一下，现在这个最好的方法是引入“睡眠队列”，让delay从**CPU占用-等待**变成彻底的**睡眠-唤醒**来。  

按照示例，在pcb结构体内加上了**ticks_sleep**字段，标记为需要睡眠的时间，每次时钟中断把所有进程的此字段减一，然后把当前进程执行时间片减一，为0时则触发调度（事实上，大多数时候都是程序主动放弃CPU,触发调度）。  

在调度函数中，每次遍历所有进程，找到sleep_ticks为0的进程，切换到其上下文；若都在睡眠中，则执行halt，让CPU陷入睡眠。  

将其实现后，却一直不可以正常执行，似乎是cpu一直卡在halt处。可是明明已经开中断了，挣扎好久，最后加上手动开中断，能够触发ticks了，但是又来一个页错误，刚开始是乱码，接着是不断闪烁，大概是触发重启，最后是清晰的页错误码。  

改了好久，一直不行，遂放弃。  
![](pictures/65.png)  
![](pictures/66.png)  

### 优化原本的阻塞

现在只能再回来考虑怎么优化我原来的这一版方案了。  

先再理清原来的**痛点**是什么：delay中是一个while循环，退出条件是延时符合预定时间，这会导致在等待期间，CPU会一直被进程占用，因此为了让其他进程也能及时的运行、并保持断言不会被触发，我把每个进程预分配的时间都设成1tick，这样能够保证在三个进程都快要达到延时退出条件时这种特殊条件（比如3000tick）都能够准确的退出系统调用。可是这样做的坏处是在系统调用延时之外也会被频繁切换上下文，切换中就容易累积误差之类的。  

那么我们实际想要每个进程怎么做？在非系统调用的其他逻辑里，尽量是连续的，而只在等待延时的系统调用里离开去做别的进程，之前是让整个进程的各个阶段都在频繁调度，现在就要只让等待阶段调度到其他地方去。  

上面那个睡眠队列虽然失败，但是给了启发：用户进程自己可以主动放弃CPU。在我前面的认知里，并没有意识到这一点，只知道内核能根据预定的时间段均匀划分，然后强制从用户进程那里“夺取”CPU，分配给其他的进程；但是现在我意识到执行schedule函数本身就是放弃CPU的实现方式：哪怕还有预分配的时间没有花完，也可以用这个函数让下一个进程用CPU。  

所以现在除了内核按时钟中断强制切换进程外，我让delay调用在while循环内也手动调用schedule，以让这“垃圾时间”留给其他进程。  

这样瞬间想通，运行，连续十几分钟，十分稳定；然后反复运行，也没有出问题。连前面鼓捣睡眠那里的东西被折磨都忘记了。  
![](pictures/67.png)  

终于完成了。  

***

# Lab6 fork和进程树

## 编写fork系统调用

一上来就要写这个，的确令人恐慌。`man fork`后出来的文档虽然没有很多，但一贯的不怎么看得进去。  

好在这次代码里的TODO十分的有情绪价值，让人比较安心。  

### 寻找空闲进程

这里在**process.h**内发现了描述进程状态的一个枚举，因此我的思路就是遍历**pcb_table**来找一个状态为可用的。  

手册说重点在于处理PCB资源不足的边界条件，得查手册……  

手册中似乎没有太多提到该怎么做，只有返回值里有提到：  
![](pictures/68.png)  

#### 阅读exec、exit

阅读时看到`do{}while(0)`的写法，没有看明白作用，搜了搜：  
+ **解决宏定义中的语法陷阱（最常用场景）**。宏定义中如果包含多条语句，直接展开可能导致逻辑错误（尤其是在 if/else 等流程控制中）。do {} while(0) 可以将宏的多条语句 “打包”，避免展开后破坏原有的语法结构。  
+ **限制局部变量的作用域**。do {} while(0) 形成的代码块可以像 {} 一样隔离局部变量，但比单纯的 {} 更灵活（尤其在宏或复杂逻辑中）。
+ **方便用 break 提前退出代码块**。在复杂逻辑中，有时需要在满足某个条件时提前结束一段代码的执行。do-while(0) 配合 break 可以实现类似 “局部返回” 的效果，比 goto 更优雅。  

这里用到的是提前退出代码块的作用。  

### 初始化

首先从手册上得知，fork最主要的就是状态机的复制；  
从exec可以得到基本流程。  

**头插法**组织已经分配的页，效率高，免得遍历整个链表。  

发现这里与之前做kmain.c中一样，分配页表、栈。  

### 返回值

写了写，就剩下返回值还不知道如何传递了。到**syscall.c**里查找，系统调用的返回接口是：  
```
static int syscall0(int syscall_nr) {
  int ret = 0;
  asm volatile("int $0x80" : "=a"(ret) : "a"(syscall_nr) : "cc", "memory");
  return ret;
}
```

可见是进了中断，符合上一次的认识。  
结合代码结构，并且查了一下，系统调用的返回值要用eax来传递，但是平时我们只管返回需要的东西就行，内核的辅助函数会完成这些脏活；而子进程就得手动设置为0了。  

### 检查

发现手册上要着重检查PCB资源不足时候的边界情况，我这里就随便写了一个返回-1，手册说文档里有说怎么做。再看看。  
手册上是多了一个“ On failure, -1 is returned  in  the  parent,  no child process is created, and errno is set to indicate the error.”  

看了一下，这里的手册是站在用户角度的，而我们写是站在内核。内核要返回的其实就是负的错误代码，然后接口会最后返回给用户态一个-1，以及设置一个用户可以找到的地方为正的错误码。  

这里也还与exit_code弄混了，这个是用于父子进程退出的，与返回值无关。  

### 额外

编译还有另一个错，是因为对进程树理解不透彻。  
```
//! \brief process inheritance tree - son
typedef struct son_node {
  struct son_node *prev;
  struct son_node *next;
  struct pcb *son;
} son_node_t;

//! \brief process inheritance tree - node
typedef struct tree_node {
  struct pcb *fa;
  son_node_t *sons;
} tree_node_t;
```
这两个结构体，下面是当前进程自身的一个定位，其父与子；子是一个双向链表，可以互相连接，适应于多个，之前理解不到位。  

运行时有bug，超过内存限制：  
![](pictures/69.png)  

可能是因为没有用maps_laddr的接口，于是再把前面写的全部更新为使用此方法。  

然后再调试，虽然还是有错，但已经不是分配内存这边的错了，而是父子进程树的构建这里。  

经典的“==”与“=”不分导致错误。。。  

这次改了后，成功运行了一次，结果后面再试，又不行了，似乎就是所谓的“奇奇怪怪的并发问题”。  

### 继续debug

十分痛苦的，找到一个可能的bug原因：pid没有分配，全部使用默认值0。  
我通过打印所有PCB的pid，发现全是0，与我预期的“PCB初始化时自动分配pid”不符：  
![](pictures/70.png)  

修改为在确认空闲进程时就按照PCB编号为其分配pid（这里也是查过，pcb与pid就是一一对应的）  

还是不行，继续。  

还有一个原因，可能是我为子进程完全复制了父进程的内核栈——这里也是我上课没有弄清楚的，内核栈是为了让内核态下清楚的知道应该为哪一个进程做事，所以每个进程都应该有完全独立的内核栈。  

关于内核栈与用户栈：我的问题在于直接复制了父进程的整个寄存器值，而其中的esp决定了内核栈的位置，导致复用；而不用mapstack是因为子进程的用户栈与父进程相同但独立，复制cr3时就会完成。  

在具体怎么做时，又犯了难，因为又涉及到一页的分配，但又经过一番研究，原来
```
typedef struct process {
  union {
    pcb_t pcb;
    u8 padding[KERNEL_STACK_SIZE];
  };
  u32 kernel_stack_bottom[0];
} process_t;
```
是自动的静态分配了8K的内核栈，其中底部被pcb信息占据，剩下的部分才是内核栈可以使用的。

一直错一直错一直错，根本没有什么解决思路。  

没办法。加上关中断，成功的概率稍微变大了，但依旧是崩溃远多于成功。  

***真的没办法了。***  

### 参考hxd的

明天就要交了，现在只好参考一下。  

一看才发现自己的误区。  

原因就在于用户内核栈这里，首先我自己的认识不清晰，在exec中看到restart的使用，误以为是全新的进程才需要使用，而fork这种复制操作不会用到。本就不确定，还去问了AI，其也给我说用不上，所以我就继续做下去了；在这现在痛苦的调试中，焦点也隐隐约约的在内核栈上，但都是不断在研究“是不是复制内核栈时哪里不对？”，忽略了这真正的结构。  
![](pictures/千问罪证)  

国产模型一生黑。。。  

实际上也是怪我，没能理解这里的东西：restart是子进程第一次被调度的“入口函数”，负责完成fork后的上下文切换衔接，**是必要的**！  

加上这个之后，立刻稳定，唉，相当于一个半星期都在为ai的错误找方法。  

## 编写wait系统调用

wait的语义上要比fork简单，也确实很快就自己写了一版，但是运行时直接有满屏幕的警告：  
![](pictures/71.png)  

一时没理解来，为什么这好好的就有这么多错？亲自看了看，原来是因为**malloc.c**中的`void phy_free_4k(phyaddr_t phy_addr)`函数内部没有完全实现，留下了一个等待填充的空白。  

那么回收，则是跟申请反着来，怎么申请来就怎么释放掉，要结合`phy_malloc_4k()`来。  

看了几下，觉得不对劲，申请物理内存时是直接从指针那里取，相当于从最顶上取，而回收时是直接点明物理页，不一定在最上面，这样会导致“空心”无法解决。  

自己模仿kmalloc使用的结构体，写了一个链表，用来存放已经释放的空闲物理页，但是运行时候，在输入、敲回车之后就卡死了。原因可能是malloc4k、fee4k这种**基础功能**，不能够使用**kmalloc**这种，基础功能必须要独立。  

所以也是没办法，根据大模型的建议，换了一种思路：在每一个物理页的开头，加上一个指向下一个空闲页的指针，这样就与kmalloc无关了。  

改了之后，依然没有动静，看来错误并不出在这里，唉，但至少减少了之后的错误概率……  
重新试了下，敲`testfork.bin`就能正常执行，看来还是wait本身没有实现好。  

重新整理了一下，成功了：  
![](pictures/72.png)  

## 运行forkbomb

不多说，直接开跑，但是这一跑才发现自己这个不太对劲，怎么一直显示**fail to find free proc**？  
![](pictures/73.png)  

原来是我的一个自定义实现，注释掉就好。  
![](pictures/74.png)  

但是forkbomb也的确是会有并发错误发生，还得再研究研究。  

发现报错有访问地址0，可能是有访问空指针。  
再排一下错，发现可能是没有清理内存，在回收僵尸进程后没有清理内存，加一个kfree即可。  

这下的确跑的更远了，通过了第一阶段测试，但是随之就是第二阶段出了错。  
![](pcitures/75.png)  

### fork中寻找空闲进程冲突

这个似乎是一直限制我在2万到3万次的原因，产生原因是当两个进程同时fork时，可能会用到同一个空闲进程，因此产生冲突，这时就需要加锁再访问。  

加上这个后，跑出了新纪录，在10w的量级，结果似乎莫名其妙进入死锁了：  
![](pictures/76.png)  

再排查一下错误，发现有可能是因为双向链表的清理不到位，重写了逻辑，这次又破了纪录，但也就是到15万的量级，唉。  

没什么好办法了，这次就这样吧，最高纪录是24万次。  
![](pictures/77.png)  

***

# lab7 os-final：signal

终于到了最终的实验，但是说来也是太懒，以及一个接一个的ddl，一直没有正式开始，拖拖拉拉。  
到现在终于是要痛定思痛、准备开始了。  

## wakatime

在文档中，助教给出了一个生产力小工具：wakatime，用于记录实际的敲代码时间，这确实高级，比之前自己预估一个时间要准得多。因此还是要配置一下。  

安装插件，到官网用github登陆，找到api-key并复制，粘贴到插件栏，就好了。  

## 信号机制

说实话，这些选题我都一点不了解的，只靠瞎蒙。所以还得是要先了解一下信号机制，究竟是个啥。  

助教给的参考链接是**IEEE POSIX**的官方标准，一看就是极其严谨极其牛逼的那种，可是满屏幕英文，搜索**signal**后蹦出来一串，还是很胆怯的。  
想了想，这会儿还是前期的了解阶段，先不看这大部头的官方文档，当务之急是先知道信号机制是用来干什么的。  

看了看这些文章：[深入理解linux信号机制](https://zhuanlan.zhihu.com/p/537431439)，大概了解了一下。  

### 简要了解

平时写程序卡死后，顺手一个ctrl c，用来kill掉进程，这就是最显式的信号机制的使用。  
信号有**三种发出方式**：终端发出（比如ctrl c）、进程发出、内核发出；而接受信号的则既有进程、又有线程。  

发出信号、接受信号之间的过程，叫做**投递**，具体的是发送到目标的**信号队列**中，这也是一个重要机制。  
不同的发送方，这个过程还是不同的。  

最后是信号的**处理**过程，是最为复杂的地方。  

### 信号类型

信号共有1-64共计64个：1-31是用**bit flag**实现的，因此称为**标记信号/标准信号/不可靠信号**，因为是由bit flag实现的，所以会发生丢失；为保证不会丢失，32-64采用**队列**来实现，不会丢失，称为**实时信号/可靠信号/排队信号**。  

### 信号的处理时机

信号是在**线程将要返回用户空间之前**进行处理的，返回用户空间有两种情况，一是从系统调用返回二是从中断返回。返回之前，线程检查队列里有没有要处理的信号。有的话就处理。  

### 信号与多线程

信号是**单线程**时代的产物，也就是当时的**进程==线程**，因此信号是线程独有、进程共享的。  
但到了多线程时代，一个进程往往有多个线程，信号究竟如何拥有？UNIX系统进行约定：  
+ 信号可以发送给进程或线程，但是同步信号（还没看懂）有限制。具体的，进程发送信号时可以选择不同的接口，能发送给进程/线程。
+ 线程信号队列的信号只能由线程自己处理，而进程信号队列里的信号由进程手下的线程处理，但具体交给谁则不确定。
+ 信号掩码的设置是线程私有的，每个线程都可独立设置信号掩码
+ 信号处理方式是全局的，后面的设置会覆盖前面的设置

再往下就是更详细的一些了，暂时还不懂，但可以先摘出来要完成的四个信号：  
1. **SIGKILL**，由内核或进程发出，发给进程，不可阻塞、不可忽略、不可捕获
2. 后面三个都在1-64个信号里面没有说明？

那么大概了解就先到此为止吧……  

## 跑起来

依旧是不知道要干啥，那么先把现有的系统跑起来。但是make时候经典的失败：  
![](pictures/78.png)  

找了一下，果然是与制作磁盘相关的指令在我这里冲突了。在*project/rules-image.mk*的第37行，加上sudo我这里就可以了。  

进入系统后，不知道有哪些命令，到user那里看一下，有一堆test命令，意外发现**test-ew**最后会卡死。  

那么基本大概模糊的了解了一下fianl-work。  

## syscall kill(缺少地基，无法实现)

既然只知道kill是有的，那么就先往系统调用方向开始搞。  
刚敲下一个**do_kill()**，却连参数怎么写都不知道，想了下，可能是现在信号机制的基础还没有打下，当然不知道怎么搞了。  

所以现在应该**先把信号机制引入**，然后才能搞上面的各种实现。  

## 把信号机制引入

信号是关于进程的，所以要到经典的proc去搞事情。  

首先，经过上面的了解，知道**信号**这一东西是**进程、线程**的属性。所以很自然的，将有关的东西塞到PCB里就很合适。  

大概看了一下，进程与线程都是用**pcb_t**来控制的，通过**tree_info.type**来区分。更具体的，每个pcb_t都是一个实例调度单元，只是根据其本质才有type。  
还可以从**tree_info_t**中看出来，清晰的标明type，可以根据type的值来分支不同的处理。  

又看了看、写了写，很清晰了：当type为0，也就是PROCESS时，这个线程就是**进程的主线程**，也就是**进程的代表**，后面那些线程则都是子线程。  
所以完全可以只定义一套信号队列、忽略的信号集，这样能符合进程/主线程、普通线程的不同需求。  

此外还纠正了一个错误理解：mask与ignore：  
+ mask是在线程级别的屏蔽信号，收到、不处理。在这方面主线程与其他线程是同等地位的
+ ignore是进程级别的忽略信号，是控制handler指向来实现的

因此最终在pcb_t里实现的是：  
```
// 信号机制新增底层
void (*signal_handler[65])(int);// 信号处理函数，进程共享，忽略/默认行为设置
sigset_t    sig_mask;           // 本线程屏蔽的信号
sigset_t    sig_pending_bit;    // 收到的标准信号 thread/process
sig_queue_t *sig_pending_que;   // 实时信号队列 thread/process
```

另外在这里信号处理函数的实现，我是很不清楚的，因为是函数指针，不咋能看的明白，所以现在弄明白：  
这是一个**函数指针数组**，每个线程都在物理上有这么一份，但是子线程并不使用自己的这个，而是每次都访问主线程/进程的那一份。  

此外，还要再建一个新文件，用于存放信号机制的一些宏定义。  
最有趣的是这个：  
```
#define SIG_DFL ((void (*)(int))0)
#define SIG_IGN ((void (*)(int))1)
```
这是**默认信号处理**与**忽略信号处理**的宏，明明是一个函数指针，却带上了值，令人困惑。  
但背后的原因~~令人暖心~~，这里就是**magic number**的地方，直接把常量强转为函数指针，其作为地址是非法的，因此实际上不能跳转执行，并且作为编译期常量，快。  
在分支处理时就可以先进行`handler==SIG_DFL`这样来判断分支。  
这个技巧是源自Unix从1970年代，POSIX标准化了它。  

## 按照流程逐一添加

现在，已经初步的把**信号机制**的底层构建出来了，接下来就要逐步完善。  
先再理清有信号的处理流程：  
1. **发送**：一个进程向另一个进程发送信号，内核为目标修改信号pending
3. **递达**：进程恢复用户态时，检查pending信号，跳转执行对应的信号处理函数
4. 继续原函数

### 进程发送信号：kill

继续理清楚概念：信号发送是一个**同步**的过程，也就是pending的修改；而递达信号是一个**异步**的操作，因为信号到达时，进程可能在执行重要操作，像中断那样去处理信号有可能造成严重后果。  
所以会先把信号加入pending，等完全有功夫去处理的时候才处理。  

所以发送信号就是一个**设置pending**的过程。而这一系统调用就是**kill**。  
> 这里为什么好端端的发送信号，非要叫**kill**呢？原因是在信号机制引入的早期，kill这一函数纯用来SIGKILL这一信号；而后期信号逐渐完善，都沿用kill的这一信号发送路径，但遵循Unix设计哲学中的“Don't break existing programs.”，所以就一直没改过。
> 这也澄清了我的前面的误解，也是顾名思义，以为要实现kill出来  

弄明白了来龙去脉，就可以把**kill**加入了。  

#### 具体实现

这里就直接到官方文档看，反而还是比较清晰的。  
所以继续搞**kill.h**去。  

#### 记一处理解错误/混乱

在阅读kill的官方文档中，发现有这么一句：  
> For a process to have permission to send a signal to a process designated by pid, unless the sending process has appropriate privileges, the real or effective user ID of the sending process shall match the real or saved set-user-ID of the receiving process.

这里用机翻看了看，讲的是**用户权限**相关的，所以很自然的联想到手册中介绍的**三级权限结构**，于是搞了一套检测当前CPL与目标进程CPL比较大小的机制，来决定是否可以发送信号。  

但是接下来才发现自己的错误理解，实验手册中提到的权限级是CPU保护环，是保护模式下的限制；而这里的描述是指**多用户**下，不同的用户进程之间是否有权力发信号。  
可是由于我们的unios没有多用户，所以本身就是一个用户的用户线程，随便发信号，不用管这些。  

因此比较CPL也是画蛇添足了，因为发送信号本身是请求内核去做的，内核高优先级，这就是白判断；而保证“高ring能向低ring发信号，低ring不能向高ring发信号”，本身就由保护模式的相关模块实现，也不用在这里手动，因此是一个误解，不用实现。  

#### 数据结构还有些问题

查了一下排队信号的处理，发现其是要按照顺序执行的：  
+ 首先是要按照**优先级**，也就是信号数值大小，越大优先级越高
+ 然后是要**先来后到**，同一类信号，先进的先处理

此外，**实时信号**还承担了**进程间通信**的作用，不只是发个信号，所以还需要一个结构去存放额外信息。  

那么现有的信号机制设计就不太合理了，首先是信号本身统一命名为**sigset_t**，但是这里指的是标准信号的位图，不适合这里；  
其次是排队队列，内部没有用来存放额外信息的结构。  

**又发现了一个认知错误，实时信号不需要用kill来传输！**  
本来发现以上这些，想着kill要有多几个版本，但没想到POSIX标准中kill就是**纯用于标准信号**，而实时信号用的是**sigqueue**。  

那么排队信号就放后面实现，现在先专注于**kill**发送标准信号。  

#### 需要发送给所有子线程吗

现在知道kill是要发送给目标进程，所以实现的语义是发到**主线程**，但是有些忧虑：难道不用发送给**所有子线程**吗？  
了解了一下，标准的语义就是如此，至于进程内部怎么处理那是后面的事情，现在发送没问题就行。  

#### 同一进程组

在pid==0时，POSIX的标准是“将sig发送给所有**与发送进程属于同一进程组（排除某些未指定的系统进程）**”，但是什么是**同一进程组**？  

又搜索了一下，在标准的OS中，每个进程除了**进程pid**，还有一个**进程组pgid**，通常等于这个组的**组长pid**。  
原因是一组相关的进程（比如一个shell命令及其子进程）可以放在同一个进程组中，便于统一管理，比如`$ sleep 100 | grep foo &`，两个命令的进程就在一组，这时候按ctrl c就把这一组都杀死了。  

**但是**，显然当前的unios并没有实现这一部分，并不支持管道（专门验证了一下）：  
![](pictures/79.png)  

所以就可以直接简化为“**自己给自己发**”。  

#### pid==-1时的“疯狂”

向所有有权发送的进程广播，其实就是全广播，无权发送的发了也白发。  

这样挺危险，极少使用，所以暂不实现。  

#### pid为其他负数

这就像是正数的**镜像**，会给**进程组pgid**为**\|pid\|**内的所有进程组广播，就和pid==0时一样，只是不是自己给自己的进程组，而是给指定进程组。  

但是依然如上，minios没有实现所谓的**进程组**，或者说每个进程都是独立的进程组，因此实际上和正的是一样的，为了贴合POSIX，也加上（反正也不要多大功夫）。  

另外，虽然在minios里没有实现，但是这里的**设计哲学**也是好的收获：  
+ 正数pid为**单个线程**
+ 负数(非-1)pid为**单个进程组**
+ -1为**所有进程**
+ 0为**自身所在的进程组**  

很优雅、细粒化的控制。  

#### 隐性实现的语义

这个挺复杂：  
+ 信号发给自己（在minios就是pid=0或pid=getpid）
+ 信号未被阻塞（不在mask里面）
+ 没有其他线程在sigwait

当满足**以上三个条件**时，就必须立刻处理信号，而不能像之前一样懒洋洋的在某一次返回用户空间前才检查。  

但是在**返回用户空间前要检查pending**的做法下，这里天生的就实现了，十分的优雅。  
实现了**把复杂性收敛到一个关键路径上**。  

#### 忽略的两段

有两段是基于**作业控制**、**会话**、**多用户**、**安全增强型系统**的，直接忽略。  

#### get_current_pl回归

发现这里的权限，包括用户权限，也包括进程的权限，所以要恢复。  
也因此要在每次访问时侯加上权限检测。  

至此，标准信号的kill就基本完成了；至于sigqueue，等标准的这里完了再看吧！  

#### 加入syscall

kill这里是没啥问题的，但是sigqueue想着也顺手加进去，但是发现其参数**sigval**作为一个联合体，转不了，得研究研究怎么转换。  

好复杂，先空下吧。  

### 修改信号屏蔽集合：sigprocmask

这里语义就比较清晰了，查阅官方文档，**sigprocmask**是针对**单线程进程**的，对多线程进程使用是一个UB；**pthread_sigmask**才是用于**线程**的信号掩码，因为信号掩码是per-thread的。  

由于minios还是支持多线程的，因此就只实现一个pthread_sigmask得了，因为最泛用了。  

首先在signal.h里加上几个宏定义，用于定义**怎么做**。  

#### 陌生关键字：restrict

发现函数原型里面有一个完全陌生的关键字：restrict。  

其是一个c99引入的**类型限定符**，只用于**指针**，目的是**帮助编译器优化代码**。  
其告诉编译器：这个指针是访问其所指向内存的唯一途径，在该作用域内，不会有其他指针alias这块内存，能帮助编译器更激进的优化，比如使用SIMD指令等。  

但这是一个程序员做出的承诺，并不一定对，如果违反了就是一个UB。  
但大差不差，应该也没啥影响，为保险起见，就不用了。  

继续注册。  

### 为指定信号设置用户态处理函数：sigaction

#### 数据结构优化

发现在标准接口中，适用的数据结构依然比我初步建立的复杂，需要重新弄一下。  
现有的结构是**hanler**，只是说明了处理对应的信号时要调用哪一个函数，但是POSIX语义是更丰富的，还包括了额外的**mask**（执行对应信号时，要屏蔽哪些信号）以及**flags**（更多的设置）。  

所以要新定义更完善的结构：**sigaction**：  
```
typedef struct sigaction_s  // 信号处理配置机构
{
    union
    {
        void (*sa_handler)(int);                        // 传统handler
        void (*sa_sigaction)(int, sig_info_t*, void*);  // 扩展handler
    };
    sigset_t sa_mask;   // 额外屏蔽的信号集
    int      sa_flags;  // 行为控制标识
}
```
有更丰富的语义，能进行更丰富的控制。  

#### 优化sig_set

同时也发现sigset的设置有点问题，现在有65个信号，实际上真正要用的是1-64号，0是一个特殊用法，可以先不考虑。这样一个uint64就能装下了，再统统右移一位，还是很好的。  
所以再优化。  
就是把原来的`1 << sig`变成`1ULL << (sig-1)`就好了。  

#### 其他的

其实干的事情差不多，只是更复杂；但是为了减小复杂度，这里还是先把高级的三参数action给禁用了，等到系统能全流程跑起来再研究别的。  

### 初步回顾

回顾一下现在干的事情，发送方设置pending已经初步完成（虽然仅实现了kill，sigqueue还没有），设置当前进程mask的信号也已经做了（直接实现了多线程进程版的，因为unios的基础是能够支持多线程的，**但是又仔细看了一下，有另一个任务是实现多线程，所以并未真正的实现，现在依旧是一个单线程进程的版本！**but可以兼容，就按先进的来），为当前进程设置对应处理函数action也已经初步实现（不支持高级的三参数handler，只有基础的，也留在后面扩充）  

但现在还没着能跑起来，因为返回用户态前检查、处理信号还没有做，对应的sigreturn也没有实现。  

所以现在先找到返回用户态前、检查并执行信号的地方，也就是**deliver**。  

### deliver

首先要确定一下deliver要插在哪里。现在已知是在**返回用户进程前**，依稀记得小作业的实现中讲过，应该是汇编类型的，但也记得不是很清晰了。  

所以大概是**kernel.asm**与**trap.asm**，把内容问ai，其分析出来就是要在kernel里面，因为kernel里面定义了三个返回函数，分别是系统调用、硬件中断、初始进程使用，忽略掉最后的这个初始化用的，前两个刚好就是前面讲的**信号处理时机**。  
而这三个最后都跳转到**restart_restore**，会恢复现场。  

因此最好插入到**reatart_syscall**和**restart_int**中，进行信号的检查、处理。  
> 涉及到这里的汇编，总令人犯难，哎。。。  

首先要回顾这两个restart的逻辑，本质上是准备好即将返回的**用户进程**、进行**调度**逻辑判断（这里一开始没懂，看了一下，是因为中断一般会跟着调度，比如最简单的时间片轮转，靠时钟中断；系统调用一般也会进行调度，比如read、write要协调io，就会主动放弃。而调度放在这里也好似因为要做的都做完了，切走安全）、**返回用户态**。  

那么检查、执行信号就插在调度判断之后、返回用户态之前了。  
信号处理，就是要让用户态去执行预定的代码，所以要修改esp，偷天换日，执行完既定的handler后再发送一个**sigreturn**，再让内核切回去。  

在研究的时候，也更清晰一些：  
+ ~~这里要构建一个函数，其接受参数是**tarpframe指针**，包括eip、cs、eflags信息，虽然这个在pcb的栈帧里也有，但是改了以后并没有什么用，因为这些信息是一个**以后要恢复的状态**，并不是**当前的状态**，只有通过指针实时修改，才能控制~~（这里越往后看越乱越糊涂，先放下）
+ 每次处理信号只处理一个，哪怕有多个信号需要处理

**有点糊涂，先放下**  

### sigreturn

这里其实就与deliver是一个**对称**的行为，deliver负责检查信号，若有，则换上一套新的mask、遵循新的flag去执行信号对应的处理函数，所以涉及到更改用户态的上下文；而执行完后要有一个return的过程，也就是恢复原来的mask，恢复用户态上下文。  

因此，要想实现这么一个功能，还需要在底层增加一个新的结构：signalframe：  
> 需要signal frame的原因就是要保留一开始的用户态的信息，防止执行handler时修改，执行完后丢失初始信息，所以要构造一个帧存起来，return时直接从这里恢复
> 也可以说在进入handler之前，把被信号打断的那个完整用户态执行现场保存起来，这样就能允许handler像普通用户函数一样随意执行、修改寄存器和栈，执行完后，通过sigreturn从signal frame 原样恢复

其结构为：  
```
typedef struct sigframe_s
{
    uint32_t      ret_addr; 
    uint32_t      signum;
    stack_frame_t regs;
    sigset_t      mask;
} sigframe_t;
```
这个结构能描述原进程的一些信息。  

跟着GPT糊糊涂涂的先弄上去。  

### ctrl c

基本结构实现了，想要验证一下功能，至少先能够通过**ctrl c**来杀死进程。  

但是键盘按下ctrl c，是通过发送一个SIGINT，这一过程发生在tty。  
具体的是**tty_keyboard_proc**这里，按照给的方法，把处理ctrl c的加进去。  

### debug

构建完基本的一套链路，跑一下，结果会黑屏，日志里是这样：  
```
[00:00:00.000][TRACE] cstart done
[00:00:00.000][TRACE] init kernel
[00:00:00.000][INFO] total memory: 255.875 MB
[00:00:00.000][TRACE] total critical memory: 2188.000 KB
[00:00:00.000][TRACE] kmem: address at 0x00224f50, 50 MB 760 KB 176 bytes
[00:00:00.000][TRACE] kpage: address at 0x034e3000, 6496 pages
[00:00:00.000][TRACE] upage: address at 0x04e43000, 45469 pages
[00:00:00.000][INFO] init memory done
[00:00:00.000][INFO] [#PF.1] trigger page fault during initializing kernel
[00:00:00.000][INFO] eip=0xc0215dc0 cr2=0x7fff0000 code=2 cs=0x000008 eflags=0x10002 from pid=-268370093
[00:00:00.000][INFO] [#PF.1] page fault handler done
```
原因是增加的那一套tempoline没能正常写入，改了改，换了几轮AI，终于是又看到亲切的控制台了。  

但是现在进入，依旧ctrl c不能用，继续排查。  

找了一圈，原因应该是，在tty中处理时，用的是`do_kill(p_proc_current->pcb.pid, SIGINT);`，这是在tty进行的，而我们知道tty是在task层上的，因此就算发送了信号，也会因为pl不符而没有什么动静。  

分析当前的结构，tty是不能杀的，但可以顺藤摸瓜，找到其子进程杀掉就好。而现在的pcb结构里就有天然的**进程树**，就在**tree_info_t**里的**child_process**。  

整个结构是：init进程fork -> fork出的子进程exec为shell，这个过程中shell会得到tty的信息，这是仅有的，需要在这里绑定。思路是给**tty_t**增加一个字段，让其知道当前对应的是哪一个shell（tty本质只是一个输入输出通道，会把捕获的传给shell去处理），然后shell再处理自己的子进程。  

那么现在已经知道shell的pid了，只需要递归杀死其所有子进程即可（之前看是说一个shell有一个进程组的，但没有实现，所以实际所谓的进程组只有一个进程；然后这一个进程执行时可能产出多个子进程，因此要杀这一个进程的所有子进程）。  

再按这个思路改一改，继续ctrl c，似乎是发送成功信号了，但是引起了崩溃，exit这里出现了问题：   
`do_exit fatal assertion failed fa_pcb->stat == READY || fa_pcb->stat == SLEEPING`  
![](pictures/80.png)  

原因是Ctrl+C 递归杀子进程时，提前改了 child_k_num 或 child_process，破坏了 shell 和子进程的 PCB 状态，导致 do_exit assert 失败。再改一改逻辑，保留父子关系，killed队列由杀手兔来维持。  
然而改后还是错，再一分析，报错原因是exit要保证父进程是ready或sleeping，应该是这里的逻辑不太对劲。  

如下图，现在很诡异的是：我为了看父进程究竟是什么状态，才导致一直出错，增加了一个打印语句，每次在assert前打印父进程状态，结果是每次都是1或2，也就是sleep与ready，按理来说是没有问题的，但是炸在了后面的lock断言：  
![](pictures/81.png)  

这就是**Heisenbug**，一观察，就变了。  
又追着gpt问了一堆，结果就是**exit**的设计问题。有设计原则是：  
+ exit必须是**幂等**的，也就是一个进程在任意状态下调用exit，都不应该崩溃，且不依赖于其他进程的状态  
+ POSIX没有规定“子进程exit时，父进程必须是ready/sleeping”，只是规定了结果，对父进程是否活着没有强制需求。  

现在的结构在事实上是实现了一个进程树上多个进程同时exit，那么exit就需要能承受乱序、重叠、交叉。  

但其实除了这个断言不符合逻辑之外，exit整体是没有什么错的。  
因此把断言注释掉，竟然**ctrl c**就没有问题了。  

### 增加简单的测试用例

到现在，ctrl c已经可以控制了，算是一个漂亮的里程碑。  

接下来就基于已经有的测试程序，扩展一下，测试看看。  

但是发现编译会报找不到我定义的信号函数。先检查一下，发现确实是没有在**lib/syscall.c**里增加定义，于是赶快加上，但是还会在编译时报错。问AI，说是要加一个声明，可是fork、exec等这些原有的似乎就没有。  

原因是fork、exec这些都是紧跟着系统诞生而有的内置api，但后面这些自己添加的就不在里面，需要手动声明。因此可以定位到**include/lib**这里。  

然后就又是一整晚的GPT询问环节。终于，算是跑通了，一次一次接近、退回、又接近、最后通了！  

### 补全标准信号

现在已经有了一条能跑通的路径，接下来就是补全。  

首先是把各个信号对应的默认动作补全，现在全都是默认exit，实际上有好几种，分别是忽略、停止、恢复、中止、中止+保存崩溃现场。  

暂停和恢复很好实现，只需要加一个状态即可，因为在schedule时只会选择READY的进程，将其变为stop本身就会忽略，再加上主动放弃CPU。  

CORE与TERM本身是一致的，都要exit，只不过core需要额外记录崩溃现场的信息，并且会给出**Segmentation fault (core dumped)**的信息。  
实现core需要有core dump文件系统支持与用户态调试器的底层，minios暂时还没有，因此就暂不实现，统统按照TERM来实现。  

然后是补全**sigaction**的**sa_flags**。  
+ **SA_NODEFER**：默认情况下进入handler时，当前信号会自动加入mask，避免handler被同一个信号重入。如果设置SA_NODEFER，则允许同一信号重入 handler。  
+ **SA_RESETHAND**：执行完handler就恢复默认，适合只进行一次的handler执行  

差不多了，测试一下新加的这个resethand，竟然挂了；但是原来那个测试就没有问题。于是再次调整，问题出在set up frame之前就已经换掉了sigaction，将其变为默认，导致处理出错。现在调整后就好了。  

接下来是测一些承压，从nodefer那里来。因为在测试程序中main会疯狂的kill来递送信号，这会导致PF的发生，还是递送的原子性+递送时机不太对。  
因此还是需要优化，引入x86.h里面的一些汇编，用来开关中断、读取cr3。  
但配了半天，还是不行。先跳过。  

然后是测试stop/cont，但是发现一旦stopped后就不能cont了，缺少某些配置。  
研究了一下，原因是在signal.c里面处理信号时，已经stopped的进程根本到不了那里，于是一stop就再也不起来了。  
解决方案是在kill的时候就进行处理。  

然后就写一个能证明标准信号是位图的测试程序，但是发现我这里依然是连续发kill的话会有些异常。  
又被GPT绕进去糊涂了半天。实际上我要实现的效果很简单：由于标准信号是位图，因此我连着疯狂kill，一定不可能完整的给我处理完。而一开始的效果不好，是因为给自己发，每次kill都是系统调用，返回是就会把信号处理了。因此用fork，让父给子发就好。  

看到文档里要求有一个利用信号机制实现一个进程同步的演示程序，简单写了一个，父子进程随机获取一个等待时间，通过发送信号同步。  

还要有能够正确接受和屏蔽信号，加一个mask就行，逻辑挺好懂的。  

### alarm

老师在布置时强调过**alarm**，但是在找资料、完成代码时并没有看到过，因此需要额外了解一下。  
**alarm**本质上是一个到点后给进程置一个SIGALARM的pending bit，可以理解为一个闹钟，闹钟一到点就给这个进程发送一个SIGALARM。  

这就比较好理解与实现了。  
在pcb_t里加一个deadline，然后主要是要在clock这里做一些工作。  
再加一个alarm的测试程序。  

### 排队信号

终于可以把这里补全了，前面都留好了空。  
但还是有些不同的，比如计划把原本的队列变为桶队列，这样在从大到小处理信号时能够更方便。  

## 测试集

+ 直接用**test-fork**来测试ctrl c的默认exit
+ 用**test-handler**来测试sigaction装载handler
+ 用**test-resethand**来测试sigaction的sa_flags标志位的SA_RESETHAND作用
+ 用**test-stop**来测试stop和cont的默认语义实现
+ 用**test-bitmap_vs_queue**来证明标准信号是bitmap
+ 用**test-sync**来演示通过信号机制同步
+ 用**test-mask**来演示正确响应信号并执行对应的处理
+ 用**test-alarm**来演示增加的alarm函数

