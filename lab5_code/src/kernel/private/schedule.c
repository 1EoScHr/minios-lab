...
void schedule(void) {
  disable_int_begin();  // 禁用中断

  /*
  process_t *cur_proc = p_proc_ready;
  process_t *next_proc = NULL;
  while (1)
  {
    for (size_t i = 0; i < NR_PCBS; i ++) // 遍历，以找到睡醒的进程
    {
      if (pcb_table[i].pcb.sleep_ticks == 0)
      {
        next_proc = &pcb_table[i];
        break;
      }
    }
    ...
    //disable_int_end();  // 如果没有睡醒的进程，则cpu睡眠，等下一个时钟中断
    //asm volatile("sti");
    //kprintf("sleep\n");
    //halt();
    //kprintf("awake\n");
  ...
  */
  process_t *cur_proc = p_proc_ready;
  process_t *next_proc = &pcb_table[(cur_proc - pcb_table + 1) % NR_PCBS];
  ...
  switch_kernel_context(&cur_proc->pcb.kernel_regs,
  ...
  // 它相当于突然被送到其他次元了，包还掉在地上（关中断），而且那个次元的时间就静止了（The
  // World！） 当在其他次元兜兜转转回来了之后，还得把包捡回来（开中断）。
