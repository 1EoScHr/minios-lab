...
void display_ELF_info(elf32_hdr_t *eh) {
  // 打印magic字段，按理说这与下面的12字节数组是合在一起为16字节的，但并没有这样做
  kprintf("magic num: %u %c %c %c\n", (char)(eh->e_magic), (char)(eh->e_magic >> 8), (char)(eh->e_magic >> 16), (char)(eh->e_magic >> 24));
  // 打印剩下的12字节
  /*
    kprintf("other e_elf[12]: ");
    for (size_t i = 0; i < 12; i ++)
    {
      kprintf("%c ", eh->e_elf[i]);
    }
    kprintf("\n");
  */
  ...
  // kprintf("e_flags    : %u\n", eh->e_flags);
  // kprintf("e_ehsize   : %u\n", eh->e_ehsize);
  // kprintf("e_phentsize: %u\n", eh->e_phentsize);
  kprintf("e_phnum    : %u\n", eh->e_phnum);        // 程序头表数
  // kprintf("e_shentsize: %u\n", eh->e_shentsize);
  // kprintf("e_shnum    : %u\n", eh->e_shnum);
  // kprintf("e_shstrndx : %u\n", eh->e_shstrndx);

  // elf32_phdr_t *segment = (void *)(ELF_ADDR + eh->e_phoff);

  elf32_phdr_t *segment = (void *)(ELF_ADDR + eh->e_phoff);
  for (size_t i = 0; i < eh->e_phnum; segment ++)
  {
    /*
      if (segment->p_type == PT_NULL) { continue; } // 为空也会计入
    */
    kprintf("segment %u: \n", i); // 段序号
    kprintf("start memory: %u\n", segment->p_vaddr);  // 在内存中起始地址
    kprintf("size of this: %u\n", segment->p_filesz); // 段大小
    kprintf("flags       : %u\n", segment->p_flags);  // 标志位
    i ++;
  }
}
...
void load_kernel() {
  ...
  elf_clus = find_kernel_elf_clus();        // 获取kernel的elf文件总扇区数

  read_data_sec((void *)ELF_ADDR, elf_clus);// 读取kernel文件
  elf32_hdr_t *eh = (void *)ELF_ADDR;       // ELF文件头表
  assert(eh->e_ehsize <= CLUSTER_SIZE);

  display_ELF_info(eh);

  load_kernel_elf(eh);                      // 加载elf文件
