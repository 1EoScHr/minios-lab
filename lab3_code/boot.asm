...
Start:
    ; 调试发现刚跳过来时eax装的还是起始扇区LBA
    mov     [PartitionLBA], eax
...
Init:
    ...
    ; 首先寻找DriverNumber，调试发现到这里dl依然是0x80，成功的保留下来了
    mov     [DriverNumber], dl

; 没有空间了，只好先把这里注释掉，因为看不见
    ; 在mbr中把分区号放在了cx中，调试也发现cx没有被动过，所以这这里就能直接打印分区序号
    ; 并且只有四个主分区序号，所以一定就是0-3，根据ASCLL关系，直接+48即可
    ;add     cx, 48
    ;mov     ax, cx
